...
<?php
							...
							{
?>
								<div class="row form result-item temp-muted-suggestions-filter <?= array_key_exists($item->getId(), $items_in_order) ? 'selected' : ''; ?> <?= $item->getMuteTemp() ? 'muted-temp' : 'unmuted-temp'; ?> <?= $item->getMutePerm() ? 'muted-perm' : 'unmuted-perm'; ?>" data-item_id="<?= $item->getId(); ?>" data-order_item_id="<?= array_key_exists($item->getId(), $items_in_order) ? $items_in_order[$item->getId()] : ''; ?>">
